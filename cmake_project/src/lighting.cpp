...
void Lighting::process(int sampleNumber, bool image)
{
    ...
    std::cout << (int)(stemp._samples.size()) << std::endl;
    ...
    if (image)
    {
        ...
        cv::imwrite("./processedData/lightings/" + bands[_band - 2] + "/" + _filename + ".jpg", img_8UC3,
            compression_params);
    }
    else
    {
        ...
        compression_params.push_back(cv::IMWRITE_PNG_COMPRESSION);
        compression_params.push_back(9);
        cv::imwrite("./processedData/lightings/" + bands[_band - 2] + "/simple_probe.jpg", img_8UC3,
            compression_params);
    }*/
}
